package cn.danlee.jt809.db;

import cn.danlee.jt809.packet.JT809Packet0x1202;
...
public class JT809Dao {
    ...
    public static int insert0x1202(JT809Packet0x1202 packet) {
        ...
        String sql = "insert into bus_gps(id,vehicleno,lon,lat,sendtime,cjsj,vec1,vec2,vec3,direction,altitude,alarm)values(?,?,?,?,?,?,?,?,?,?,?,?)";
        int insert = 0;
        try {
    ...
    public static int delYesterdayData()  {
        QueryRunner runner = new QueryRunner();
        Connection conn = DataSourceConnectionFactory.getDbConnection();
        String sql = "DELETE FROM bus_gps WHERE sendtime < CURRENT_DATE";
        int del = 0;
        try {
